...
export function LoadingScreen({ onEnter }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const increment = currentProgress > 80 ? Math.random() * 1.5 : Math.random() * 3 + 1;
  ...
  const handleEnterClick = () => {
    if (!containerRef.current || !ringGroupRef.current || !contentRef.current) return;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <svg className="w-[400px] h-[400px] -rotate-90" viewBox="0 0 300 300">
      ...
      {/* Central Content */}
      <div ref={contentRef} className="relative z-10 flex flex-col items-center justify-center h-[300px] w-[300px]">
